...
def list_existing_ids_for_source_file(
    client: QdrantClient, collection_name: str, source_file: str
) -> set[str]:
    ids = set()
    offset = None
    while True:
...
def run() -> None:
    ...
    for row in rows:
        ...
        candidates.append(
            {"id": pid, "text": text, "payload": payload, "source_file": row["source_file"]}
        )
    ...
    changed_or_new = [
        c for c in candidates if existing_fingerprints.get(c["id"]) != c["payload"]["fingerprint"]
    ]

    stale_ids = []
    if collection_exists:
        for source_file, current_ids in ids_by_source_file.items():
            existing_ids_for_file = list_existing_ids_for_source_file(
                client, QDRANT_COLLECTION, source_file
            )
    ...
    if changed_or_new:
        ...
        points = [
            models.PointStruct(
                id=changed_or_new[i]["id"], vector=vectors[i], payload=changed_or_new[i]["payload"]
            )
